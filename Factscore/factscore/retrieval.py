...
class DocDB(object):
    ...
    def get_text_from_title(self, title):
        ...
        return results
    ##Hyeon
    def get_text_from_titles(self, titles):
        """Fetch the raw text of the documents for a list of 'titles'."""
        cursor = self.connection.cursor()
        results = []
        ...
        for title in titles:
            ...
            # Ensure that a result was found for the title
            assert fetch_result is not None and len(fetch_result) == 1, f"`topic` in your data ({title}) is likely to be not a valid title in the DB."

            # Process each paragraph separated by a special separator and add it to the results
            for text in fetch_result[0][0].split(SPECIAL_SEPARATOR):
                results.append({"title": title, "text": text})

        cursor.close()
        assert len(results) > 0, "No valid titles found in the database."
        return results

    def get_all_titles(self):
        """Fetch and return all document titles from the database."""
        cursor = self.connection.cursor()
...
class WikiRetrieval(object):
    ...
    def get_passages_wiki(self, topic, question, k):
        ...
        if cache_key not in self.cache:
            ...
            ##Hyeon
            passages = self.db.get_text_from_titles(topic)
            if self.retrieval_type=="bm25":
                self.cache[cache_key] = self.get_bm25_passages(topic, retrieval_query, passages, k)
            elif self.retrieval_type.startswith("gtr-"):
...
class BioRetrieval(object):
    ...
    def get_bio_passages(self, topic, retrieval_query, passages, k):
        if self.encoder is None:
            self.load_encoder()
        # if topic in self.embed_cache:
        #     passage_vectors = self.embed_cache[topic]
    ...
    def get_passages_bio(self, topic, evidence, question, k):
        #retrieval_query = topic + " " + question.strip()
        retrieval_query = question.strip()
        #cache_key = topic + "#" + retrieval_query
        cache_key = retrieval_query
        ...
        self.cache[cache_key] = self.get_bio_passages(topic, retrieval_query, passages, k)
        assert len(self.cache[cache_key]) in [k, len(passages)]
        self.add_n += 1
