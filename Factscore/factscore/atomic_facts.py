...
# def text_to_sentences(text):
#     sentences = text.split("- ")[1:]  # Splits the text at "- " and discards the first element, which is likely empty or unnecessary.
#     sentences = [sent.strip() for sent in sentences if sent.strip()]  # Strips and filters out empty strings.
    
#     # Now that we have only non-empty strings, we process them.
#     if sentences:
#         sentences = [sent[:-1] if sent[-1] == '\n' else sent for sent in sentences]  # Remove trailing newline character if present.
#         if sentences[-1][-1] != '.':  # Ensure the last sentence ends with a period.
#             sentences[-1] += '.'
#     else:
#         sentences = []  # Reinitialize to an empty list if all were filtered out.
    
#     return sentences

# This function should now handle cases where input lines could be empty more gracefully.
