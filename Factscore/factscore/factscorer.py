...
from factscore.retrieval import DocDB, WikiRetrieval, BioRetrieval
...
class FactScorer(object):
    ...
    def register_knowledge_source(self, name="enwiki-20230401", db_path=None, data_path=None):
        if name=="enwiki-20230401":
            ...
            cache_path = os.path.join(self.cache_dir, f"wikiretrieval-{name}.json")
            embed_cache_path = os.path.join(self.cache_dir, f"wikiretrieval-{name}.pkl")

            self.db[name] = DocDB(db_path=db_path, data_path=data_path)
            self.retrieval[name] = WikiRetrieval(self.db[name], cache_path, embed_cache_path, batch_size=self.batch_size)
            if "npm" in self.model_name:
                cache_path = os.path.join(self.cache_dir, f"wikibm25-{name}.json")
                embed_cache_path = os.path.join(self.cache_dir, f"wikibm25-{name}.pkl")
                self.npm[name] = NPM(WikiRetrieval(self.db[name], cache_path, embed_cache_path, "bm25"),
                                    "npm-single",
                                    cache_file=os.path.join(self.cache_dir, f"npm-{name}.pkl"))
        else:
            cache_path = os.path.join(self.cache_dir, f"bioretrieval-{name}.json")
            embed_cache_path = os.path.join(self.cache_dir, f"bioretrieval-{name}.pkl")

            self.retrieval[name] = BioRetrieval(cache_path, embed_cache_path, batch_size=self.batch_size)
            if "npm" in self.model_name:
                cache_path = os.path.join(self.cache_dir, f"biobm25-{name}.json")
                embed_cache_path = os.path.join(self.cache_dir, f"biobm25-{name}.pkl")
                self.npm[name] = NPM(BioRetrieval(cache_path, embed_cache_path, "bm25"),
                                    "npm-single",
                                    cache_file=os.path.join(self.cache_dir, f"npm-{name}.pkl"))
    ...
    def get_score(self,
        ...
        if "ChatGPT" in self.model_name:
            ...
            total_words = 0
            for topic, evidence, generation, facts in zip(topics, evidences, generations, atomic_facts):
                if facts is not None:
                    if knowledge_source=="enwiki-20230401":
                        total_words += self._get_score_wiki(topic, generation, facts, knowledge_source, cost_estimate=self.cost_estimate)
                    else:
                        total_words += self._get_score_bio(topic, evidence, generation, facts, knowledge_source, cost_estimate=self.cost_estimate)
        ...
        for topic, evidence, generation, facts in zip(topics, evidences,generations, atomic_facts):
            if facts is None:
                ...
                if knowledge_source=="enwiki-20230401":
                    decision = self._get_score_wiki(topic, generation, facts, knowledge_source)
                else:
                    decision = self._get_score_bio(topic, evidence, generation, facts, knowledge_source)
    ...
    def _get_score_wiki(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None):
        decisions = []
        total_words = 0
        for atom in atomic_facts:
            atom = atom.strip()
            if self.lm:
                passages = self.retrieval[knowledge_source].get_passages_wiki(topic, atom, k=5)
                #print(passages)
    ...
    def _get_score_bio(self, topic, evidence, generation, atomic_facts, knowledge_source, cost_estimate=None):
        decisions = []
        total_words = 0
        for atom in atomic_facts:
            atom = atom.strip()
            if self.lm:
                passages = self.retrieval[knowledge_source].get_passages_bio(topic, evidence, atom, k=5)
                #print(passages)
                definition = "Answer the question about {} based on the given context.\n\n".format(topic)
                context = ""
                for psg_idx, psg in enumerate(reversed(passages)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--knowledge_source',
                        type=str,
                        default="enwiki-20230401",
                        help="enwiki-20230401 or biomedical")
    ...
    tot = 0
    topics, generations, atomic_facts, evidences = [], [], [], []
